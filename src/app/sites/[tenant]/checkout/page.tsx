        ...
        return (
          <div key={label} className='flex items-center gap-2'>
            <div className='flex flex-col items-center gap-1'>
              ...
              <span
                className='text-[9px] font-medium tracking-[.06em]'
                style={{ color: isOn ? 'var(--taupe)' : 'var(--tx-faint)' }}>
                {label}
              </span>
            </div>
            {i < labels.length - 1 && (
              <div
                ...
                style={{
                  background: s < step ? 'var(--taupe)' : 'var(--line)',
                }}
              />
            )}
          </div>
...
function StepCustomerData({
  ...
  onNext: (data: CustomerDataForm) => void;
}) {
  const {
    register,
    handleSubmit,
  ...
  return (
    <form onSubmit={handleSubmit(onNext)} className='space-y-3'>
      ...
      <label
        className='flex items-start gap-2.5 text-[11.5px] cursor-pointer'
        style={{ color: 'var(--tx-soft)' }}>
        <input
          type='checkbox'
          {...register('subscribeToNewsletter')}
          style={{ accentColor: 'var(--taupe)', marginTop: 1 }}
        />
        ...
        {[
          {
            ...
            sub: '2 a 5 días hábiles',
            price: 4500,
          },
          {
            id: 'retiro' as const,
            label: 'Retiro en sucursal',
            sub: '3 a 7 días hábiles',
            price: 0,
          },
        ].map((o) => (
          <button
            ...
            <span
              ...
              style={{
                border: `2px solid ${method === o.id ? 'var(--taupe)' : 'var(--line)'}`,
              }}>
              {method === o.id && (
                <span
...
function StepPayment({
  ...
  const methods = [
    ...
    },
    {
      ...
      sub: 'Débito inmediato',
      badge: 'Mobbex',
    },
    {
      id: 'transfer',
      label: 'Transferencia bancaria',
      sub: 'Generá tu talón de pago',
      badge: 'Talo',
    },
  ];
  ...
  return (
    <div className='space-y-3'>
      <p
        ...
        style={{ fontFamily: 'var(--font-head)', color: 'var(--marron)' }}>
      ...
      {methods.map((m) => (
        <button
          ...
          <span
            className='w-[18px] h-[18px] rounded-full flex items-center justify-center flex-shrink-0'
            style={{
              border: `2px solid ${paymentMethod === m.id ? 'var(--taupe)' : 'var(--line)'}`,
            }}>
            {paymentMethod === m.id && (
              <span
                className='w-2 h-2 rounded-full'
  ...
  return (
    <div>
      ...
      {React.cloneElement(children, {
        id,
        className:
          'w-full rounded-[10px] px-3 py-3 text-[13px] outline-none transition-all',
        style: {
...
export default function CheckoutPage({
  ...
  async function handleOrder(paymentMethod: string) {
    ...
    try {
      ...
      if (!res.ok) {
        setApiError(
          json?.error?.message ?? 'Ocurrió un error al procesar tu pedido.',
        );
        return;
      }
    ...
    <div className='max-w-5xl mx-auto px-4 py-8'>
      ...
      <div className='max-w-lg md:max-w-none'>
        <div className='flex items-center gap-3 mb-2'>
          <button
            onClick={() =>
              step > 1 ? setStep((s) => (s - 1) as 1 | 2 | 3) : router.push('/')
            }
